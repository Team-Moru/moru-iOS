...
final class SwiftDataHomeWeatherRepository: HomeWeatherRepository {
  ...
  func cachedWeather() throws -> HomeWeatherSnapshot? {
    ...
    var validSnapshots: [(model: PersistedHomeWeatherSnapshot, snapshot: HomeWeatherSnapshot)] = []
    var didMutate = false

    for persisted in persistedSnapshots {
      do {
        let snapshot = try SwiftDataV2Mapper.makeHomeWeatherSnapshot(
          from: persisted,
          now: currentDate
        )
  ...
  func saveWeather(_ snapshot: HomeWeatherSnapshot) throws {
    ...
    let persistedSnapshot = try SwiftDataV2Mapper.makePersistedWeatherSnapshot(
      from: snapshot,
      now: currentDate
    )
    ...
    existingSnapshots.forEach { modelContext.delete($0) }
  ...
  func eraseCachedWeather() throws {
    ...
    persistedSnapshots.forEach { modelContext.delete($0) }
    try modelContext.save()
  }
