...
struct RoutinePlayerView: View {
    ...
    private func summaryView(
        ...
        RoutineFinishedView(
            ...
            // 오늘의 기록 화면을 NavigationStack에 push
            onTapTodayRecord: {
                isShowingTodayRecord = true
            },

            // 현재 RoutinePlayer fullScreenCover를 닫고
            // 기존 HomeView로 돌아감
            onTapHome: {
        ...
        .navigationDestination(
            ...
            TodayRoutineRecordView(
                ...
                // 실제 기상 시각이 아직 별도로 저장되지 않으므로
                // 현재는 루틴 시작 시각을 사용
                wakeUpTime: summary.startedAt,

                results: viewModel.stepResults,

                // NavigationStack에서 완료 화면으로 pop
                onTapBack: {
                    isShowingTodayRecord = false
                },

                // 전체 RoutinePlayer 화면을 닫고 HomeView로 이동
                onTapHome: {
                    viewModel.requestSummaryExit()
                }
            )
            // TodayRoutineRecordView가 자체 상단 헤더를 사용하므로
            // 시스템 뒤로 버튼은 숨김
            .navigationBarBackButtonHidden(true)
        }
    }

    /// 루틴 시작 시각과 완료 시각의 차이를 초 단위로 변환합니다.
