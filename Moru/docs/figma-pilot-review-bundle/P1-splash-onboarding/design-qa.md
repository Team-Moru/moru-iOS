...
- P0: AX3 exact line-height와 고정 footer에서 progress, 제목, CTA 글리프가
  잘리거나 겹쳤다.
  - 수정: 접근성 크기에서 자연 line-height를 사용하고 한 줄 CTA에 축소
    안전장치를 적용했다.
...
- P1: CodeRabbit 접근성 리뷰에서 progress와 footer CTA의 Dynamic Type 상한,
  장식 이미지의 접근성 노출, loading splash의 진행 상태 부재를 지적했다.
  - 수정: 두 상한을 제거하고 progress label은 AX 크기에서 자연 line-height로
    확대되게 했다. CTA는 최소 54 pt에서 내용에 따라 커지며 한 줄 전체 문구를
    유지한다.
  - 수정: clock, organizing halo, completion badge를 접근성 트리에서 숨겼다.
  - 수정: loading 상태를 idle과 분리하고 작은 system activity indicator와
    VoiceOver의 `앱을 준비하고 있어요` 상태값을 제공했다.
- 최종 13상태 × Medium/AX3 재캡처에서 progress와 CTA의 영구
  clipping/overlap이 없음을 직접 확인했다.
...
- Accessibility: Medium/AX3 13상태에 영구 clipping/overlap이 없고 본문은 scroll로
  접근 가능하다. progress와 CTA도 AX3까지 확대되고, 버튼은 semantic Button과
  기존 accessibility identifier를 유지한다. 장식 이미지는 숨기고 loading 상태는
  시각적 activity indicator와 VoiceOver에 전달한다.
...
- Before/After MAE 범위 `5.586668446790019...31.36947270452595`은 Splash asset,
