...
- 화면 제목은 `마이`에서 Figma의 `설정`으로, 음성 행은 `음성 설정` 아래
  `모루 말투`로 보정했다.
- 로컬 프로필 카드, 번들 음성 선택 sheet, 알람 상태와 조치, 데이터 초기화 및
  확인 dialog를 유지했다.
- Profile Figma의 계정 surface 대신 실제 로컬 프로필임을 명시하고,
  기능이 없는 로그인·로그아웃·회원탈퇴는 노출하지 않았다.
