...
| 6 | 필수 | WeatherKit을 제거한 실제 제출 빌드를 업로드·선택한 뒤 version/build, bundle ID, device family, minimum OS와 최종 entitlement·링크 상태 확인 | `privacy-and-review-ko.md`, `support-ko.md` | Developer App ID의 Capabilities·App Services 양쪽 WeatherKit 해제, 새 배포 프로파일과 실제 제출용 archive | 빌드·출시 책임자 | ◐ 진행 중 | 1.0(7) 최종 IPA에서 위치 권한 문구, WeatherKit entitlement·framework·심볼 없음과 새 배포 프로파일 UUID `c7f70f19-0588-4b5a-950c-306ed7b8d83d` 확인. 2026-08-07 13:10 KST 업로드 성공. App Store Connect 처리 완료·빌드 선택과 실기기 확인 필요 |
...
| 13 | 필수 | App Review 담당자 연락처와 WeatherKit 미지원 Review Notes 입력, 반려 메시지에 미지원 사실 답변 | `privacy-and-review-ko.md` | 실제 연락처 확정, 1.0(7) archive 제거 검증, Review Notes 365 UTF-8 bytes 확인 | 심사 책임자 | ☐ 미완료 | 담당자명·이메일·전화번호, Review Notes 저장 화면과 1.0(7) 빌드 선택 확인. WeatherKit 기능 녹화는 첨부하지 않음 |
...
공개 지원 사이트와 1.0(7) 빌드 업로드는 완료했습니다. 토큰의 실제값 교체·검증, App Store Connect 메타데이터 입력, 처리된 1.0(7) 빌드 선택과 제출은 아직 미완료입니다.
