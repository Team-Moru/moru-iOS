...
> **Draft API 연동 주의:** 이 문서의 데이터 비전송 답변은 2026-08-05 업로드
> 빌드 기준입니다. 빠른 실행 API 연동 draft는 로그인 상태에서 루틴 구성,
> CHECK 전사문, INPUT 자유 입력, 단계 완료 결과를 MORU 서버로 전송합니다.
> 원본 음성 파일은 이 변경으로 전송하지 않지만, draft를 출시 빌드에 포함하기
> 전 개인정보 처리방침과 App Privacy 답변을 다시 확정해야 합니다. 아래의
> `수집 없음` 문구를 새 빌드의 제출 답변으로 그대로 재사용하면 안 됩니다.
> 앱이 현재 연결하는 공개 문서(`https://team-moru.github.io/privacy`,
> `https://team-moru.github.io/terms`)도 서버 전송·계정 사용이 없다고 설명하므로
> 이 draft와 일치하지 않습니다. 공개 문서 수정과 사용자 고지, App Privacy
> 재신고가 완료되기 전에는 이 변경을 출시 빌드에 포함하지 않습니다.
