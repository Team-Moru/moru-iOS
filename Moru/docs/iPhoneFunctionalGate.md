...
- 직접 진입에서 알람 정지는 RoutinePlayer 표시가 승인된 뒤에만 수행한다.
  다른 화면이 떠 있어 미뤄진(`deferredBusy`) 알람은 끄지 않고, 그 화면이 닫힌 뒤
  같은 봉투로 다시 들어와 그때 끈다. 정지에 실패하면 플레이어 상단 배너로
  재시도를 제공하고 루틴은 그대로 진행한다.
...
- 상단바 닫기(X)와 종료는 둘 다 지금까지의 결과를 저장한다. 닫기는 요약 없이 홈으로,
  종료는 완료 요약 화면으로 간다. 시작 직후 이탈도 0% 실행 기록으로 남긴다(유예 없음).
- 기록 저장이 실패해도 나가는 길은 막지 않는다. 배너의 "다시 시도"는 같은 요청으로
  재시도하고, 닫기·종료·"기록 없이 나가기"는 저장하지 못한 기록을 버리고 나갈지 묻는다.
  단계 완료·건너뛰기 게이트(`isStepInteractionDisabled`)는 그대로 유지한다.
- 완료 요약은 계획된 모든 단계를 완료·건너뜀·미완료로 보여 준다. 저장되는 결과는
  완료·건너뜀만이며 미완료는 표시용이다.
- 루틴이 떠 있는 동안 자동 잠금을 막는다. 백그라운드를 다녀오면 안내를 다시 틀지 않고
  음성 인식만 자동으로 다시 켜며, 타이머는 벽시계 경과분을 한 번에 차감한다.
- 타이머 단계는 남은 시간과 상관없이 "완료했어요"로 끝낼 수 있다.
...
- 로컬 TTS fallback, 키보드 입력은 추가하지 않는다. 손으로 완료하는 버튼은 음성 인식이
  영구 실패한 단계(권한 거부, 기기·한국어 미지원)와 타이머 조기 완료에만 둔다.
- STT 침묵 자동 종료 기준은 3초다.
  마지막 transcript를 자동 완료 판정에 전달한다.
- 음성 인식 권한 거부 시에는 설정 이동, 해당 단계 완료 버튼, 건너뛰기를 제공한다.
  일시 실패(네트워크·오디오 세션·인식 오류)는 다시 시도만 제공한다.
- `intro 완료 → STT 시작`,
