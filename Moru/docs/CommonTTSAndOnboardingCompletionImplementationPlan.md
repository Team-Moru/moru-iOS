...
> **상태: 구현 완료 (2026-09-11 확인).** 이 문서는 실행 계획이 아니라 완료된
> 작업의 설계 기록입니다. `GET /tts`는 `AccountServerTarget.swift:24`에,
> `POST /onboarding/complete`는 `ProductionRoutineSyncRequestPreparer`의
> `completeOnboarding` command로 연결돼 있습니다. 아래 "새 작업 세션 시작 지침"이
> 오래 "1단계부터 구현하라"고 지시하고 있었으나, 그대로 따르면 이미 있는 기능을
> 다시 만들게 됩니다.
...
## 이 문서를 다시 열 때 (2026-09-11 갱신)

구현이 끝났으므로 "1단계부터 구현한다"는 옛 지시는 삭제했습니다. 이 기능을
고치러 왔다면 아래 순서를 따릅니다.

1. 이 문서는 설계 의도와 완료 조건을 확인하는 용도로만 읽는다.
2. `git status --short`로 사용자 변경사항을 확인하고 보존한다.
3. 실제 동작은 코드에서 확인한다. 이 문서 작성 이후 변경됐을 수 있다.
4. 서버 계약을 추측해 바꾸지 않는다. 불일치는 실제 병합 코드·응답으로 재확인한다.
5. 고친 뒤에는 focused test와 전체 gate를 실행한다.
