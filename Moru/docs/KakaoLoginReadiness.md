...
Debug와 Release의 저장소 기본값은 실제 credential로 오인되지 않는 아래
placeholder다.
...
Native app key는 공개 식별자지만, 실제 값과 console 등록 상태를 확인하기
전에는 저장소에 추정 값을 넣지 않는다. Admin key, client secret, 토큰은 앱
configuration에 추가하지 않는다.

## 출시 blocker

- Kakao Developers의 실제 Native app key
- Kakao Developers iOS 플랫폼의 `com.teammoru.Moru` bundle ID 등록 확인
- Kakao 로그인 활성화와 필수 동의항목 설정 확인
- 개발·운영 앱 분리 여부와 각 환경의 Native app key·callback scheme 확정
