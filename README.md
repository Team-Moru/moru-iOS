...
> 검증된 원격 TTS 안내 음원과 로컬 fallback, 확인형·입력형 단계의
> 음성 인식(STT)으로 자연스럽게
> 아침 루틴을 진행하도록 돕습니다.
...
- 로그인 계정의 서버 binding과 검증된 로컬 캐시가 준비되면 RoutinePlayer의
  `intro`는 루틴별 원격 TTS를, `done`·`remind`는 선택 음성별 공통 원격 TTS를
  우선 재생합니다. 서버 `intro` 캐시가 없으면 첫 재생 전에 제한된 준비 시간 동안
  준비 화면을 표시하고 기존 다운로드를 유지한 채 최대 30초 동안 생성과 다운로드를
  기다립니다. 제한 시간 안에 준비되지 않으면 백그라운드 재시도를 예약하고 루틴은
  무음으로 진행합니다. 서버 공통 음원의 캐시·재생이 실패한 경우에도 로그인
  세션에서는 무음으로 진행해 다른 번들 목소리가 섞이지 않게 합니다.
  로그인하지 않은 로컬 루틴만 네 종류의 번들 MP3를 fallback으로 사용하며,
  로컬 합성 TTS fallback은 제공하지 않습니다.
- STT는 지원되는 루틴 단계의 자동 완료에 사용합니다.
  3초 침묵 뒤 마지막 transcript를 판정하며, 권한 거부·기기 미지원처럼 다시 시도해도
  회복되지 않는 실패에서만 손으로 완료하는 버튼을 제공합니다(권한 거부는 설정 이동 포함).
  타이머 단계는 남은 시간과 상관없이 완료할 수 있습니다.
- 루틴 실행 중에는 닫기(X)·종료 모두 지금까지의 결과를 저장하고, 저장에 실패해도
  "기록 없이 나가기"로 나갈 수 있습니다. 화면 자동 잠금을 막고, 백그라운드를 다녀오면
  음성 인식과 타이머를 되살립니다. 상세 계약은 기능 게이트 문서의
  "루틴 실행 탈출 계약"을 따릅니다.
